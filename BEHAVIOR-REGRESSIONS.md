...
当前独立回归测试套件包含 150 个测试且全部通过（含 Phase 0 新增的 39 条解码器对抗用例与 12 条语料库完整性用例）。伴唱展开测试还验证了连续的进出高度以及无独立滑动的有界缩放。指针进入/移动/移出事件由常驻激活的跟踪区域（tracking area）处理；指针移出默认立即启动模糊补间。模糊逻辑对当前所有活跃或保留的并行前景行予以豁免，而间奏行与常规历史行依然会正常模糊。窗口尺寸调整重新排版（resize reflow）采用临界阻尼、零初始速度的轨道，使新折行的半行能顺畅贴合新几何结构，而不会产生焦点切换的反弹晃动。三个间奏点采用居中的变换锚点，并通过 `interludeDotScale` 暴露直径大小。当指针在视图内时，手动浏览悬停保持挂起；指针移出后，在预设超时（当前播放器预设为 5 秒）后恢复自动跟随，且采用相同的有序弹簧级联动画而非生硬重置。
