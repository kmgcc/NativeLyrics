...
软件包测试套件目前包含 99 个 XCTest 用例。它覆盖了 TTML 解码、AMLL 绝对时间轴、显式 W3C 相对时间轴、元数据、歌词翻译、罗马音、Ruby 注音、对唱/伴唱图层、布局排版、有界字形缓存（bounded glyph cache）、时钟语义、窗口缩放重排（resize reflow）、焦点移动、跳转定位（seek）行为、悬停交互行为、入场/退场动画、重音强调、发光（glow）、动态模糊、合成通道以及公开的计时与配置契约。
